...
int main()
{
	...
	if(rc)
	{
		std::cout << "Failed to Open Database: " << stderr << std::endl;
		return(0);
	}
	else
	{
	...
	//Execute Table
	rc = sqlite3_exec(db, sql, sqlite_callback, 0, &ErrorNote);
	
	if(rc != SQLITE_OK)
	{
		std::cout << "Error During Table Creation: " << stderr << std::endl;
	}
	else
	{
